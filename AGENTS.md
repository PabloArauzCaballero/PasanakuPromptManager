...
Este repo es el **estándar de ingeniería, diseño y operación de Pasanaku**: 194 skills, 16 reglas
y los candados que las hacen cumplir. Si sos un agente de IA trabajando en cualquier repo de
Pasanaku —o un programador usando uno—, esto es lo que tenés que respetar, sin importar qué
herramienta uses.

Dos hechos del producto mandan sobre todo lo demás, y por eso están arriba: **Pasanaku administra
dinero de terceros** (regla 91) y **está construido como microservicios** (regla 98).
...
### 1.4 Dinero — prohibiciones duras

Esta plataforma mueve plata de gente que confía. No admiten excepción operativa (regla 91):

- **Prohibido `float`/`double` para dinero**, en cualquier capa. Decimal exacto y **siempre** con
  su moneda ISO-4217 al lado.
- **Prohibido editar o borrar** un asiento, un movimiento contable, una bitácora o un movimiento
  de fondo: son append-only. Se corrige con **contra-asiento**, nunca con `UPDATE`.
- **Prohibida una operación de dinero sin clave de idempotencia** con índice único en base. Un
  reintento del proveedor, del usuario o de la cola no puede acreditar dos veces.
- **Prohibido que un saldo salga de una suma ad-hoc.** Todo total se calcula desde el mayor.
- **Nada se promete antes de estar cobrado**: solo acredita el webhook firmado y verificado.
- **Prohibido castigar sin debido proceso**: mora, sanción, ejecución de aval y baja de reputación
  exigen expediente, plazo de descargo y estado `FIRME`.
- **El organizador no cobra comisión ni es cuenta de paso** (RN-18): es una restricción de
  arquitectura, no una preferencia.

### 1.5 Datos personales y financieros — prohibiciones duras
...
- **Nunca en la URL** (path ni query): quedan en historiales, proxies y logs de acceso — y en
  microservicios, también en los del gateway y de cada salto.
...
- **Prohibido copiar datos de producción** a desarrollo o prueba. Si necesitás volumen, generá
  datos sintéticos. **Prohibido restaurar un backup de producción en staging sin anonimizar.**
- **Nunca el valor plano de un token, OTP o credencial**: solo su hash con *pepper*.
- Si una salida que tenés que pegar los contenía: se enmascara **y se aclara que se enmascaró**.

### 1.6 Microservicios — prohibiciones duras

Regla 98 completa en `.claude/rules/98-microservicios.md`:

- **Un servicio es dueño de sus datos.** Prohibido leer o escribir la base de otro, ni con una
  vista, ni "solo para un reporte".
- **El contrato se versiona y no se rompe.** Quitar, renombrar o cambiar la semántica de un campo
  es incompatible: versión nueva, convivencia y fecha de retiro declarada.
- **No hay transacciones distribuidas.** Prohibido 2PC; un flujo de dinero que cruza servicios es
  saga + outbox, con compensaciones idempotentes.
- **Todo consumidor es idempotente**: la entrega es at-least-once.
- **Ningún servicio confía en que "el gateway ya validó".** Cada uno autentica y autoriza.
- **Prohibida una llamada saliente sin timeout explícito.**
- Un E2E verde con todos los servicios sanos **no es evidencia de resiliencia**: cerrar exige
  haber apagado un dependido a propósito.
...
> `CUPO_NO_DISPONIBLE` en el interceptor de errores del front. Quedó en
> `feature/cupo-conflictos`, compila, E2E `cupo-conflicto` en rojo por esto.
...
| Qué | Dónde |
|---|---|
| **Las 16 reglas completas** | [`.claude/rules/`](.claude/rules/) — empezá por [`README.md`](.claude/rules/README.md) |
| **Catálogo de 194 skills** | [`.claude/skills/`](.claude/skills/) |
| **Modelo de datos de Pasanaku** | [`Pasanaco_backendBO/docs/Index.md`](../Pasanaco_backendBO/docs/Index.md) — nueve módulos, una ficha por entidad |
