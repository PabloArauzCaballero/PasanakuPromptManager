...
- **Criterio de aceptación (CA)** — *qué* tiene que ser cierto para el usuario o el sistema.
  ...
  > CA: Dado un grupo con un solo cupo libre, cuando dos participantes lo toman al mismo tiempo,
  > entonces uno queda dentro, el otro recibe 409 `CUPO_NO_DISPONIBLE` y la UI le ofrece otro grupo
  > sin perder lo que había cargado.
...
- **Definition of Done (DoD)** — *cómo se demuestra*, y qué más tiene que estar en verde.
  ...
  > DoD: `yarn test cupos` en verde (pegar salida) · test de concurrencia real (dos peticiones en
  > paralelo) PASS · E2E `cupo-conflicto` PASS con trace · migración aplicada · sin datos sensibles
  > en el log del 409 · revisado contra `security-guardrails` y `money-movement-safety`.
