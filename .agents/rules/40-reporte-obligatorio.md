...
> Ejemplo aceptable: "H2.S1.M3 — El webhook rechaza la firma inválida y el test unitario pasa. La
> acreditación todavía no es idempotente: dos entregas del mismo evento crean dos asientos (lo
> reproduje, salida en `evidencia/webhook-duplicado.txt`); falta el índice `UNIQUE` sobre
> `clave_idempotencia` y capturar la violación. Quedó en `feature/webhook-idempotente`, compila,
> test `webhook-duplicado` en rojo por esto."
...
5. **Sin datos sensibles.** Ninguna salida pegada puede contener datos reales de participantes,
