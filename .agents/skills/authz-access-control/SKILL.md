...
| 2 | ¿Su rol puede ejecutar **esta función**? | BFLA — un participante invoca un endpoint de admin |
...
- **Ownership**: el recurso es del actor (su perfil, sus aportes, sus publicaciones).
- **Relación** (ReBAC): el actor tiene un vínculo vigente con el dueño — el profesional que
  **atiende** a ese participante, el miembro de esa organización, el titular del seguro. La
...
- En dinero la regla típica combina todo: *rol profesional* **y** *relación de atención vigente*
  **y** *consentimiento que cubre ese dato* (ver `consent-management`). El rol solo no alcanza:
  ser organizador no da acceso a todos los participantes.
...
1. Todo endpoint con un identificador en ruta, query o body lo verifica. También los anidados:
   `/members/:pid/records/:rid` exige que `rid` **pertenezca** a `pid`, además del acceso a `pid`.
...
4. Recurso ajeno ⇒ **404**, no 403, cuando la sola existencia es información (¿este participante
   tiene historia en esta financiera?). 403 cuando el actor ya sabe legítimamente que existe.
...
6. El identificador del actor sale del token/sesión; **nunca** de un campo del body o un header
   que el cliente controla (`userId`, `organizerId`, `X-User`).

```ts
// ❌ autenticado ≠ autorizado sobre ESTE aporte
const appt = await em.findOneOrFail(Contribution, id);

// ✅ el actor es parte de la condición; si no es suya, no existe
const appt = await em.findOne(Contribution, {
  id,
  $or: [{ member: actor.personId }, { professional: actor.personId }],
});
...
- **Salida**: DTO de respuesta **por vista y por rol**, nunca la entidad del ORM serializada.
  Devolvé lo mínimo que esa pantalla necesita (`data-privacy-financial`). Un mismo recurso puede tener
...
| Recurso · acción | Anónimo | Participante | Profesional | Admin org. |
|---|---|---|---|---|
| Aporte · leer | ✗ | propia | donde es tratante | de su organización |
| Aporte · cancelar | ✗ | propia, si estado lo permite | propia calendario | ✗ |
...
4. Relación inexistente o vencida (profesional que no atiende a ese participante) → 404/403.
...
- Aceptar `userId`/`tenantId` del body. 403 que confirma la existencia de un expediente financiero.
