...
await em.transactional(async (tx) => {
  const appt = await tx.findOneOrFail(Contribution, id, { populate: ['slot'] });
  appt.confirm(actor);          // la regla vive en la entidad/dominio
  tx.persist(new ContributionEvent(appt, 'confirmed', actor));
...
```typescript
const rec = await em.findOneOrFail(LedgerEntry, id, {
...
- Para setear una FK sin leer la fila: `em.getReference(Organizer, organizerId)`. No hagas un
  `findOne` solo para asignar una relación.
- N+1 típico: iterar una lista y tocar una relación lazy. Declará `populate` en la query.

```typescript
// ❌ 1 + N
const appts = await em.find(Contribution, { day });
for (const a of appts) names.push((await a.member.load())!.fullName);

// ✅ populate declarado; estrategia explícita si el join explota filas
const appts = await em.find(Contribution, { day }, {
  populate: ['member', 'slot.organizador'],
...
- Proyectá con `fields` en listados; no traigas columnas pesadas (texto financiero, blobs)
