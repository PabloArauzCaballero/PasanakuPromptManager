...
Dos `string` son intercambiables para el compilador; un `MemberId` y un `OrganizerId` no
deberían serlo.
...
export const MemberId = (raw: string): MemberId => {
  if (!UUID_RE.test(raw)) throw new InvalidIdError('MemberId', raw);
  return raw as MemberId; // único `as` permitido: dentro del constructor validado
};
```
...
- Fechas: un instante es `Date`/ISO-8601 con zona (UTC en tránsito y en base). Una fecha
  ...
  un `Date` — si no, la zona horaria le corre el día. Ver `contribution-calendar`.
