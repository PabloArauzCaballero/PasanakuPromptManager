...
```bash
gh pr create --draft --base develop --title "fix(calendario): …" --body-file pr.md
...
```bash
gh issue list --search "solapamiento de turnos" --state all   # buscar duplicados ANTES de crear
...
- [ ] Acciones de la columna derecha de §8: confirmación humana obtenida y aporteda.
