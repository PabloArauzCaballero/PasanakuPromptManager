...
| **Spike** | Salto brusco | Ráfagas: apertura de inscripciones, envío masivo de notificaciones |
...
2. Modelá **flujos**, no endpoints sueltos: login → listar calendario → abrir aporte → guardar. Con
...
export const options = {
  scenarios: {
    calendario: { executor: 'ramping-vus', stages: [
...
export default function () {
  const res = http.get(`${__ENV.BASE_URL}/contributions?limit=20`, { headers: { Authorization: `Bearer ${__ENV.TOKEN}` } });
