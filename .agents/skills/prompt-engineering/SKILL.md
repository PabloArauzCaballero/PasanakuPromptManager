...
❌ `Revisá el módulo de aportes.`
✅
```text
Revisá src/contributions en busca de condiciones de carrera al confirmar un aporte.
...
❌ `No devuelvas el documento completo del participante.`
✅ `Devolvé solo los campos que la vista muestra: el resto es dato financiero que esta pantalla no tiene por qué recibir, y cada campo extra es superficie de fuga.`

## 3. Rol y contexto

- Una frase de rol en el system prompt enfoca tono y criterio ("Sos revisor de seguridad de una API financieros multi-tenant"). Que sea específico al trabajo, no un título grandilocuente.
...
documentos extensos, pedí primero los aportes relevantes y después el análisis sobre esas aportes.
