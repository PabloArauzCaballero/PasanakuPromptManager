...
```ts
@WebSocketGateway({ namespace: 'contributions', cors: { origin: ALLOWED } })
export class ContributionsGateway implements OnGatewayConnection {
  ...
  @SubscribeMessage('contribution.subscribe')
...
- Una **sala** (room) es una lista de suscripción, **no** un control de acceso por sí sola. Antes
  de `join`, verificá que el usuario tiene derecho a ese recurso (que el aporte/chat es suyo o tiene
  relación de atención).
- Nombrá las salas con el tenant y el recurso: `tenant:{t}:contribution:{id}`. Nunca dejes que el
...
- No loguees payloads con datos sensibles ni tokens (`data-privacy-financial`).
