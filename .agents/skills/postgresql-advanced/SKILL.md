...
description: PostgreSQL en profundidad — leer `EXPLAIN (ANALYZE, BUFFERS)`, elegir índice (B-tree, GIN, GiST, BRIN), búsqueda con `pg_trgm`, `unaccent` y full-text, exclusion constraints para calendario sin solapes, locks y `lock_timeout`, `CREATE INDEX CONCURRENTLY`, aislamiento y `40001`, Row Level Security, JSONB, particionado, vacuum y pool. Usar al diagnosticar una query lenta, diseñar un índice o búsqueda, aplicar DDL sobre tablas con tráfico, aislar tenants o garantizar una invariante en la base.
---
...
CREATE INDEX organizador_name_trgm ON organizador USING gin (f_norm(full_name) gin_trgm_ops);
...
## 4. Exclusion constraints: calendario sin solapamientos
...
ALTER TABLE contribution ADD CONSTRAINT contribution_no_overlap
  EXCLUDE USING gist (
    organizer_id WITH =,
...
- La violación llega como SQLSTATE **`23P01`** (`exclusion_violation`): mapeala a 409
  con código estable (ver `error-handling-contract`). Ver `contribution-calendar`.
...
```sql
ALTER TABLE financial_note ENABLE ROW LEVEL SECURITY;
ALTER TABLE financial_note FORCE ROW LEVEL SECURITY;   -- sin esto, el dueño de la tabla la saltea
CREATE POLICY tenant_isolation ON financial_note
