...
description: Contrato de errores de la API — errores de dominio y su mapeo a HTTP (400, 401, 403, 404, 409, 412, 422, 428, 429, 5xx) con criterio, problem details (RFC 9457), códigos estables para el cliente, traducción de errores de base y ORM, exception filter único en NestJS, y nada de stack, SQL ni datos de participantes en la respuesta. Usar al crear una excepción de dominio, decidir qué status devuelve un caso, escribir o revisar un exception filter, documentar errores en OpenAPI, o cuando el front no distingue dos fallos.
---
...
- El dominio lanza excepciones **propias** (`CupoNoDisponibleError`), sin conocer HTTP.
...
- **403 vs 404 sobre recurso ajeno**: si revelar que existe es una fuga (historial del participante
  de otro participante, recurso de otro tenant), respondé **404**. Decidilo por tipo de
...
```json
{
  "type": "https://errors.example.com/contribution-overlap",
  "title": "El horario ya no está disponible",
  "status": 409,
  "code": "CUPO_NO_DISPONIBLE",
  "detail": "El profesional tiene otra aporte entre 10:00 y 10:30.",
...
- **PII financiera**: documento, nombre de participante, diagnóstico, email de un tercero. Ni en
  `detail` ni en el log del error (ver `data-privacy-financial`). Referí por id opaco.
...
if (isUniqueViolation(e, 'uq_contribution_request')) throw new DuplicateRequestError(requestId);
...
- [ ] Sin stack, SQL, nombres de constraint ni PII financiera en el cuerpo ni en el mensaje logueado.
