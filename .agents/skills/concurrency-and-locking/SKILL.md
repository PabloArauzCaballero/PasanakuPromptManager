...
| No solapar rangos (turnos, bloqueos de periodo) | `EXCLUDE USING gist` + manejar `23P01` |
...
|---|---|---|
| Cuándo | Conflictos raros; ediciones de usuario que duran minutos (formularios, ficha financiera) | Conflictos frecuentes sobre la misma fila; sección crítica corta dentro de una transacción |
...
- `FOR UPDATE`: bloquea la fila hasta el commit. Bloqueá la **fila padre** que serializa
  el recurso (el calendario del grupo, la cuenta contable), no cada hijo.
...
aporte, registrar pago, emitir asiento, enviar solicitud) acepta `Idempotency-Key`.
...
- **Doble toma del último cupo**: dos participantes toman el mismo hueco. → `EXCLUDE` sobre
  `(organizer_id, rango)`; el perdedor recibe 409 con código estable y la UI recarga huecos.
  Un `SELECT` de disponibilidad previo es solo cortesía.
- **Bloqueo de calendario vs aporte existente**: el bloqueo y el aporte se crean a la vez. →
  misma constraint de exclusión cubriendo ambos tipos, o lock de la fila de calendario.
