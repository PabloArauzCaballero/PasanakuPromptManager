...
| Invariante que debe valer **ya** (saldo, cupo del grupo, unicidad) | sincrónico, en la misma transacción |
...
- **Evento**: hecho ya ocurrido, en pasado ("AporteConfirmada"). Cero o N consumidores. No se rechaza.
...
- Un evento **no lleva datos de más**: identificadores y lo mínimo; el consumidor autorizado
  consulta el resto (`data-privacy-financial`).
...
await em.transactional(async (tx) => {
  ...
  tx.persist(new OutboxMessage({ type: 'contribution.confirmed', version: 1,
    aggregateId: contribution.id, tenantId, payload: { contributionId: contribution.id } }));
});
...
- No asumas orden global. Si importa, importa **por agregado** (todas las cosas de *esta* aporte).
...
interface EventEnvelope<T> {
  ...
  type: string;          // 'contribution.confirmed' — pasado, dominio.hecho
...
- [ ] Payload mínimo, sin datos innecesarios, validado con esquema al consumir.
