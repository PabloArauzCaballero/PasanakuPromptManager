...
```sql
-- ✅ entidades financieros dentro de 5 km del punto dado (usa idx GiST)
SELECT id, name
FROM facility
...
Para "las 10 farmacias más cercanas a mi desembolso", ordená por el operador de distancia `<->`, que
...
La ubicación de una persona es dato sensible; combinada con dinero, más aún (`data-privacy-financial`).

- **Minimizá**: guardá la ubicación solo si el feature la necesita, y con la **precisión mínima**
  que sirva. Para "hay servicios en tu zona" no necesitás la posición exacta del participante:
  redondeá/aproximá.
...
- La ubicación de un participante no viaja en logs, URLs ni notificaciones (`notifications-delivery`).
- Distinguí ubicación **pública** (dirección de un punto de cobro publicada) de **privada** (dónde
...
- Ubicación del participante en logs/URLs; API key sin restringir; ignorar términos del proveedor.
