...
- **Estado de servidor**: datos que viven en el backend (calendario, ficha, catálogos). Se *cachean*,
...
class FilterNotifier extends Notifier<CalendarioFilter> {
  @override
  CalendarioFilter build() => const CalendarioFilter.today();
  void setRange(DateTimeRange r) => state = state.copyWith(range: r);
}
final filterProvider = NotifierProvider<FilterNotifier, CalendarioFilter>(FilterNotifier.new);

// Estado de servidor (asíncrono): carga/dato/error los da AsyncValue
class ContributionsNotifier extends AsyncNotifier<List<Contribution>> {
  @override
  Future<List<Contribution>> build() =>
      ref.watch(contributionRepoProvider).findForDay(ref.watch(filterProvider).day);
}
final contributionsProvider =
    AsyncNotifierProvider<ContributionsNotifier, List<Contribution>>(ContributionsNotifier.new);
```

En la UI, resolvé los tres casos con `when`, sin `bool` sueltos:

```dart
ref.watch(contributionsProvider).when(
  data: (items) => ContributionList(items: items),
  loading: () => const ContributionsSkeleton(),
  error: (e, _) => ErrorRetry(onRetry: () => ref.invalidate(contributionsProvider)),
);
```
...
- Considerá clases de valor con igualdad por valor (p. ej. `freezed` o `==`/`hashCode` propios;
  verificá el paquete en pub.dev antes de aporterlo).
...
final container = ProviderContainer(overrides: [
  contributionRepoProvider.overrideWithValue(FakeContributionRepo()),
]);
