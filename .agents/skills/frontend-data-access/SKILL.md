...
- Una clase de acceso a datos por recurso del backend (`MemberApi`, `ContributionApi`),
...
export class ContributionApi {
  ...
  getPage(cursor?: string): Observable<Page<Contribution>> {
    let params = new HttpParams();
    if (cursor) params = params.set('cursor', cursor);
    return this.http.get<Page<Contribution>>(`${this.base}/contributions`, { params });
  }
}
```
...
class ListComponent {
  ...
  ngOnInit() { this.http.get<any[]>('/contributions').subscribe(x => this.items.set(x)); }
}
```
...
- Nunca loguear el body del error si puede traer datos de participante (ver `frontend-error-monitoring`).
