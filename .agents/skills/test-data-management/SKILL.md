...
| **Del test** | El participante, el aporte, el asiento que este test ejercita | El propio test, vía factory | Dura el test |
...
const member = fixtures.members[3];

// ✅ factory con defaults válidos + override de SOLO lo relevante para este test
const member = await make.member({ birthDate: '2010-05-01' });   // el test es sobre un menor
const appt = await make.contribution({ member, status: 'requested' });
```
...
- **Unicidad sin azar ciego**: sufijo derivado de un contador o de `workerIndex` + id de corrida
  (`participante-w2-0007@test.invalid`). Si usás un generador tipo faker, fijá la **semilla** y
...
- Casos de calendario obligatorios en dominios con calendario: fin de mes, año bisiesto, cambio de
  horario, medianoche, zona horaria del usuario ≠ zona del servidor (`contribution-calendar`).
...
1. Prohibido copiar producción a test, staging o local. Tampoco "solo una tabla", tampoco
   ...
   de `data-privacy-financial` con responsable, no una decisión de un test.
