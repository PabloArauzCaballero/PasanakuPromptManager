...
1. **Lecturas de datos sensibles**, no solo escrituras: abrir un expediente, ver un
   documento, buscar a una persona, exportar o imprimir. En dinero, "quién vio" importa tanto
   como "quién cambió".
2. Toda mutación de datos financieros, financieros, de identidad y de consentimiento.
...
- **No copies datos sensibles/PII completa al log de auditoría**: convierte al log en el blanco más jugoso
...
- Ver `data-privacy-financial` para clasificación y enmascarado.
...
| **Inmutable + nueva versión** | El registro no se edita; una enmienda es una fila nueva que referencia a la anterior | Documentos financieros, asientos contables, firmas (`rosca-group-lifecycle`, `accounting-double-entry`) |
...
- Volcar el JSON completo del participante en cada evento.
...
- [ ] Sin datos sensibles/PII completa, secretos ni cuerpos crudos en el log.
