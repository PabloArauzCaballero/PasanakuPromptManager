...
Dibujá el flujo con estos elementos: **actores externos** (participante, organizador, banco),
...
| Info disclosure | Autorización por objeto, respuesta mínima, cifrado, datos sensibles fuera de logs | `authz-access-control`, `data-privacy-financial` |
...
- Priorizá por impacto × probabilidad. En dinero, cualquier amenaza de **Information disclosure**
  o **Elevation** sobre datos de participantes es prioridad máxima por defecto.
- Las amenazas no mitigadas se convierten en casos de prueba para el ejercicio de evaluación y en
  tests de `security-testing`.

## 5. Foco por feature (ejemplos de dónde mira STRIDE en Pasanaku)

- **Pago QR y webhook**: Spoofing (webhook sin firma), Tampering (monto alterado), Repudiation
  ("yo no pagué" / "yo sí pagué"), Elevation → `payments-qr-integration`.
- **Entrega y desembolso**: Tampering (cuenta bancaria cambiada antes de cobrar), Repudiation
  (quién ordenó), DoS sobre el punto de no retorno → `disbursement-payouts`.
- **Sorteo de turnos**: Tampering (semilla conocida de antemano), Repudiation (nadie puede
  recomputar el orden) → `rosca-group-lifecycle`.
- **Fondo de garantía y avales**: Elevation (cubrir sin límite), Tampering (aval sin aceptación)
  → `guarantee-fund-workflows`.
- **Mora y sanciones**: Repudiation (no se puede demostrar la notificación), Info disclosure
  (exponer la deuda de alguien) → `collections-delinquency`.
- **Grupos públicos y emparejamiento**: Info disclosure antes del consentimiento, enumeración y
  scraping → `group-discovery-matching`, `consent-management`.
- **Llamadas entre servicios**: Spoofing (identidad inyectada), Elevation (confiar en el
  gateway) → `service-to-service-security`.
