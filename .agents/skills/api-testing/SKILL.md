...
it('GET /members/:id devuelve el recurso', () =>
  request(app.getHttpServer()).get(`/members/${id}`).set('Authorization', `Bearer ${token}`).expect(200));
```
...
it.each([
  ...
  ['otro usuario',     tokens.memberB,   404],
  ['otro tenant',      tokens.tenantB,   404],
])('PATCH /records/:id — %s → %i', async (_n, token, status) => {
  const req = request(app.getHttpServer()).patch(`/records/${recordOfOrganizerA}`).send(patch);
  if (token) req.set('Authorization', `Bearer ${token}`);
  await req.expect(status);
  expect(await readRecord(recordOfOrganizerA)).toEqual(original);   // no cambió NADA
});
```
...
en `test-data-management`. Nunca datos reales (`data-privacy-financial`).
