...
| Dato | Regla |
|---|---|
| Respuestas de datos financieros o PII | no en caché compartido; si hace falta, solo por usuario, TTL corto, cifrado en reposo, y `Cache-Control: no-store` hacia el cliente (`data-privacy-financial`) |
...
| Saldos, cupos del grupo, contadores que deciden una escritura | nunca: se leen en la transacción que decide (`concurrency-and-locking`) |
...
5. **Cliente** (signals/stores en Angular, store en Flutter): `frontend-data-access`,
   `mobile-offline-sync`. Misma regla: datos sensibles con cuidado y limpiar al cerrar sesión o cambiar tenant.
...
@Get('me/contributions') list() { /* ... */ }

// ✅ respuesta por actor y tenant, TTL corto, sin exponerla a intermediarios
const key = `api:v3:${ctx.tenantId}:contributions:list:${ctx.actorId}:${hash(query)}`;
const cached = await this.cache.get<ContributionListDto>(key);
...
- [ ] Nada financiero ni autorizado en caché compartido; `no-store`/`private` por defecto en rutas autenticadas.
