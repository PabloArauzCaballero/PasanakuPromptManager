...
acoso, contenido dañino, suplantación. En un producto financiero el riesgo sube: desinformación
administrativa y datos sensibles expuestos por los propios usuarios. Complementa `authz-access-control`
(quién puede qué) y `transparency-reputation` (el modelo del feed).
...
| Revisión humana | decidir lo ambiguo, el acoso, el matiz financiero | no escala a todo el volumen |
...
- Cuidado con el sesgo del filtro (dialectos, idiomas, términos del dominio legítimos marcados como
  ofensivos). Medí falsos positivos.
- Contenido generado o clasificado por IA **no** es verdad contable ni juicio definitivo
  (`money-movement-safety`, `anti-hallucination-guard`): es una señal para un humano.
...
- Un usuario puede publicar **su propia** información financieros o la de otro. No puedes impedirlo
...
- No expongas en superficies públicas datos que el sistema conoce del usuario (que es participante de
  X, su diagnóstico): eso es fuga tuya, no del usuario (`data-privacy-financial`).
...
- Filtro automático como juez final de contenido ambiguo; IA tratada como verdad contable.
