...
description: Método para bajar normativa de protección de datos y dinero a controles de ingeniería verificables — identificar los marcos aplicables con el responsable legal, matriz requisito → control técnico → evidencia → dueño, privacidad por diseño y por defecto, evaluación de impacto, registro de tratamientos, gestión de brechas y revisión periódica. Usar al arrancar un producto o módulo que trate datos personales o financieros, al recibir un requisito "de cumplimiento", al preparar una auditoría o evaluación de impacto, y al responder "¿esto cumple?" sin inventar la respuesta.
effort: high
---
...
legal aporteda de memoria vale lo mismo que una API inventada.
...
3. Los marcos de referencia sirven como **catálogo de controles**, no como afirmación de que
   aplican. Ejemplos habituales para datos financieros: GDPR (categoría especial de datos financieros,
...
| Campo | Ejemplo |
|---|---|
| Tratamiento | «Calendario de aportes con especialidad» |
| Finalidad | Prestación del servicio financieros |
| Categorías de datos | Identificación, contacto, dato financieros (especialidad, motivo) |
| Categorías de titulares | Participantes, profesionales |
...
PR que agrega un campo o un flujo (`data-privacy-financial` §1, `technical-docs-and-adr`).
...
|---|---|---|---|---|---|---|
| R-07 | «Registrar todo acceso a datos financieros» | Evento de auditoría en cada lectura | `audit-trail-history` | Query de eventos por participante + test de integración | Backend | IMPLEMENTADO |
...
- «Control organizativo» (política, capacitación, contrato) también tiene fila y dueño, aunque
  no sea código.
- Cada control apunta a la skill de la casa que lo detalla: privacidad (`data-privacy-financial`),
  consentimiento (`consent-management`), expediente financiero (`rosca-group-lifecycle`), acceso
...
3. **Separación**: identificación separada de datos financieros donde sea viable; seudonimización
   en analítica.
...
6. **Terceros**: todo proveedor que reciba datos personales tiene contrato de tratamiento vigente
   antes de la primera llamada. Sin contrato, el dato no sale (`data-privacy-financial` §3).

## 5. Paso 4 — Evaluación de impacto

Cuando un tratamiento es de alto riesgo (datos financieros a escala, perfilado, nuevas tecnologías,
...
3. Riesgos para las personas (no para la empresa): reidentificación, acceso indebido, pérdida,
   discriminación, daño por error financiero.
