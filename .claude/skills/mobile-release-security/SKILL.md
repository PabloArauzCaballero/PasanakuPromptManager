...
- Crash reporting y analytics: **jamás** datos sensibles, tokens ni identificadores financieros en eventos,
  breadcrumbs o mensajes de error. Filtrá/allow-list los campos que se envían.
- Logs de release al mínimo; nada de volcar respuestas de la API con datos de participantes.
  Ver `data-privacy-financial` y `backend-observability` (mismo criterio de no-PII en logs).
