...
| H1.S2.M1 | El conflicto devuelve 409 `CUPO_NO_DISPONIBLE` | test de API del conflicto | PASS — `evidencia/m4-api.txt` |
```
...
- Qué falta exactamente: agregar la entrada `CUPO_NO_DISPONIBLE` al mapa de códigos
  del interceptor de errores y su texto; el mapa ya existe y tiene tres entradas.
- Dónde quedó: rama `feature/calendario-conflictos`, compila y typecheck en verde;
  E2E `cupo-conflicto` en rojo únicamente por esta aserción.
```
...
**Datos sensibles.** Ninguna salida puede llevar datos reales de participantes, secretos ni tokens.
...
```text
409 CUPO_NO_DISPONIBLE profesional=<ID-ENMASCARADO> franja=10:00-10:30
(participante y documento enmascarados manualmente para este reporte)
```
...
```markdown
# Reporte — Impedir doble toma del último cupo
...
- [ ] Ninguna salida pegada tiene datos de participantes, secretos ni tokens; lo enmascarado, declarado.
