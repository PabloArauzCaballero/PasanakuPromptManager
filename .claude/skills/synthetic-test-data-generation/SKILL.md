...
(aislamiento, limpieza, reloj) lo cubre `test-data-management`, la privacidad `data-privacy-financial`,
...
- Prohibido copiar datos de producción a un entorno de prueba. Si necesitás forma real, generá
  contra el modelo, no contra la base productiva. Datos financieros reales jamás salen de producción.
...
- Generá el grafo, no filas aisladas: un `participante` con sus `aportes`, cada aporte con un
  `profesional` y un `horario` que existan y no se solapen.
- Respetá las FKs y las reglas: un aporte no puede ser anterior a la fecha de alta; un asiento
...
- Proporciones plausibles: si el 5% de los aportes se cancelan, generá ~5%, no 50%.
...
## 5. Dominio dinero (sintético)
- Nombres/documentos/direcciones: `faker` localizado. Nunca un documento de identidad real.
- Datos financieros: valores plausibles dentro de rango fisiológico si el test lo requiere, pero
  claramente ficticios; nunca notas del expediente reales. No inventes dosis ni interacciones
  (ver `money-movement-safety`).
