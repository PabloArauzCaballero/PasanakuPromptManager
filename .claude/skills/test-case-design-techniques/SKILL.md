...
|---|---|---|---|
| organizador | sí | — | sí |
| organizador | no | sí | sí |
| organizador | no | no | no |
| admin | — | — | no |

- Una fila = un caso. Colapsá filas donde una condición es indiferente (`—`).
- Cubre combinaciones que "a ojo" se olvidan (el organizador no dueño sin consentimiento).

## 4. Transición de estados
Para entidades con ciclo de vida (aporte, cotización, solicitud): tomá la máquina de
