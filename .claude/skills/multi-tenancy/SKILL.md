...
Un tenant es la organización dueña de los datos (financiera, grupo organizador, banco). La falla
característica no es un crash: es **mostrarle a una organización los datos de otra**, en
silencio. En dinero eso es una brecha de datos financieros. `authz-access-control` decide qué
...
ALTER TABLE contribution FORCE ROW LEVEL SECURITY;   -- también aplica al dueño de la tabla
...
em.addFilter('tenant', (args) => ({ tenant: args.tenantId }), [Contribution, Invoice]);
...
- El participante suele ser transversal a organizaciones: decidí explícitamente qué datos suyos son
  del tenant (la historia en esa financiera) y cuáles de la persona (su perfil), según
  `consent-management`.

## 9. Operación

- Alta de tenant idempotente y scripteada; baja con política de retención y borrado definida
  (`data-privacy-financial`). Restore por tenant: planteá cómo se hace **antes** de necesitarlo
  (`backup-restore-dr`).
