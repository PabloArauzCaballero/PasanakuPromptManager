...
```typescript
const msg = $localize`:@@aporteAcreditado:Tu aporte fue confirmada`;
```
...
<span i18n>{gender, select, female {Organizera} male {Organizer} other {Profesional}} disponible</span>
```
...
- Concatenar fragmentos traducibles (`'Hola ' + name + ', tenés ' + n + ' aportes'`).
