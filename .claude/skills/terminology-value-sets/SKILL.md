...
En un sistema financiero casi todo lo que parece «un enum» es en realidad **terminología**: tiene
...
Mismo vocabulario que EMVCo ISO 20022 (`CodeSystem`, `ValueSet`, `ConceptMap`) — ver
`payments-qr-integration`. Un concepto puede pertenecer a muchos value sets; un value set
no «posee» conceptos.
...
| Se persiste en el registro | Siempre (vía concepto) | Como **copia histórica** cuando el registro es financiero o legal |

- Un código retirado se marca inactivo; **no se reutiliza** con otro significado ni se borra.
- En registros financieros guardá además el display mostrado y la versión: lo que el profesional vio
  es parte del registro — ver `rosca-group-lifecycle` §4.
...
**Validación de pertenencia** (equivalente a `$validate-code` en un servicio terminológico ISO 20022):
...
ISO 4217 CT (financiera general), códigos de banco (laboratorio y observaciones), ICD/CIE (clasificación de
enfermedades), ATC (clasificación de entregas). **Licencias, ediciones nacionales y
...
No cargues catálogos financieros desde fuentes no oficiales ni generada por IA — ver
