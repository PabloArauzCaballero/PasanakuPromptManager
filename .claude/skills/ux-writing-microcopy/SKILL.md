...
- **Consistente**: un concepto, una palabra en todo el producto (no mezclar
  "aporte"/"turno"/"consulta" para lo mismo — decidí y respetá).

## 2. Botones y acciones

- El botón nombra su acción, no "Aceptar/OK": `Guardar cambios`, `Enviar solicitud`,
  `Eliminar participante`. Al leer solo el botón se sabe qué va a pasar.
...
❌  "Entrada inválida."
❌  "Error: ECONNREFUSED"
✅  "No pudimos guardar el aporte: ese horario ya está ocupado. Elegí otro horario."
```
...
- El vacío orienta y ofrece la primera acción: "Todavía no tenés aportes. Creá la primera."
...
- Títulos que dicen qué es la pantalla, no dineroos huecos ("Bienvenido a tu dashboard").
...
- El diálogo nombra el objeto y la consecuencia: "¿Eliminar el aporte de María López del
...
- Definí una voz (cercana pero profesional, en un producto financiero: seria, respetuosa,
  sin chistes con datos financieros) y sostenela.
- Nada de humor ni ansiedad en contextos sensibles (resultados, pagos, errores financieros).
- Escribí pensando en traducción: evitá modismos difíciles de localizar, no concatenes
  frases con variables en medio ("Tenés " + n + " aportes" se rompe en plurales/género —
