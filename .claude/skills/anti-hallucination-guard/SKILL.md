...
1. Buscá por **nombre y por sinónimos**, en español y en inglés, singular y plural (`aporte`, `turno`, `contribution`, `booking`).
...
❌ Crear `OrganizerScheduleDto` porque "hace falta uno" → ya existía `AvailabilitySlotDto` usado por tres endpoints.
✅ "Busqué `schedule|availability|horario|calendario` en `src/**/dto` y en el OpenAPI: existe `AvailabilitySlotDto` (`ruta:línea`). Lo extiendo con `breakMinutes`."
...
- Si el requisito exige datos reales (instituciones, especialidades, entregas, bancos, divisiones administrativas), no generes valores plausibles. Requerí fuente y registrá procedencia (ver `seed-data-catalogs`).
- No infieras información financiera: dosis, contraindicaciones, equivalencias (ver `money-movement-safety`).
