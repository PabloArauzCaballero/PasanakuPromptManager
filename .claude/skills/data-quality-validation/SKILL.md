...
```sql
SELECT contribution_id FROM scheduling.contributions
...
```sql
SELECT a.contribution_id, a.status_concept_id
  FROM scheduling.contributions a
 WHERE NOT EXISTS (SELECT 1 FROM terminology.value_set_members m
                    WHERE m.value_set_code = 'contribution_status'
...
**Solapamientos** — dos turnos del mismo periodo que se pisan (lo que un `EXCLUDE` impediría):

```sql
SELECT a.contribution_id, b.contribution_id
  FROM scheduling.contributions a
  JOIN scheduling.contributions b
    ON a.resource_id = b.resource_id AND a.contribution_id < b.contribution_id
...
| Todo acceso a expediente tiene consentimiento vigente | anti-join acceso ↔ consentimiento (`consent-management`) |
...
- Las salidas pueden contener PII financiera: reportá IDs y conteos, no contenido (`data-privacy-financial`).
