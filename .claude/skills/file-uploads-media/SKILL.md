...
`security-guardrails` (visión general) y `data-privacy-financial` (los adjuntos sensibles son datos sensibles).
...
- El acceso a un archivo pasa por **tu** control de acceso, igual que cualquier recurso: quién
  pide, tiene relación con ese recurso (`authz-access-control`, y para financieros la relación de
...
- La URL firmada vence pronto (minutos), es de un solo recurso y no se loguea con el token
  (`data-privacy-financial`: nada sensible en URLs que queden en logs/historial).
...
- Imágenes con `sharp`: `resize` con límites máximos, `toFormat`/`toBuffer` a un formato seguro,
  descartando metadatos EXIF (que pueden traer geolocalización del participante — privacidad).
...
## 6. Adjuntos financieros = datos sensibles

- Una radiografía, un PDF de laboratorio o una foto de una lesión son datos financieros: aplican
  **todas** las reglas de `data-privacy-financial` y `rosca-group-lifecycle` (inmutabilidad, auditoría de
  cada acceso, retención, borrado).
- Registrá cada **lectura** de un adjunto financiero en el rastro de auditoría (`audit-trail-history`).
- Datos de prueba: nunca subas archivos con datos reales a entornos de desarrollo (`test-data-management`).

## 7. Ciclo de vida

- Borrado: al eliminar el recurso dueño, borrá (o marcá para borrar) el objeto; evitá huérfanos
  que acumulan costo y riesgo. Para financieros, seguí la política de retención, no borres a lo loco.
...
- Servir sin escanear; datos sensibles o token en la URL; no auditar el acceso a adjuntos sensibles.
...
- [ ] Adjuntos financieros con reglas de privacidad: auditoría de acceso, retención, inmutabilidad.
