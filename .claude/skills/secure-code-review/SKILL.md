...
2. **Autorización**: cada acceso por id, ¿valida ownership/tenant/rol en el backend?
   (`authz-access-control`, `multi-tenancy`). Es la clase más grave en fintech.
...
5. **Salida y logs**: ¿se loguea datos sensibles, tokens, payload crudo? (`data-privacy-financial`).
...
grep -rnE "console\.(log|error)\(|logger\.(log|debug)\(.*(user|member|body)" src/
