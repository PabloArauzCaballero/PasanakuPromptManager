...
- **Evento de dominio** — "AporteConfirmada". No sabe nada de canales.
...
@OnEvent('contribution.confirmed')                       // consumidor de la cola
async notify(e: ContributionCreditedEvent) {
  await this.notifications.dispatch({
    template: 'contribution.confirmed', recipientId: e.memberId,
    dedupeKey: `appt-confirmed:${e.contributionId}`, data: { contributionId: e.contributionId },
  });
}
...
|---|---|---|
| Accionable urgente (recordatorio de aporte en 1h) | push + in-app | email si no hay push |
...
- Cada notificación lleva una **`dedupeKey`** estable derivada del hecho, no del intento:
  `appt-reminder:{contributionId}:{scheduledFor}`. Un `INSERT` con `UNIQUE(recipient, dedupeKey, channel)`
...
## 7. Privacidad — el cuerpo no lleva datos financieros

Regla dura (`data-privacy-financial`): el contenido de una notificación puede aparecer en una pantalla
bloqueada, en un preview de email, en logs del proveedor y en su retención.

- ❌ "Tu resultado de VIH está listo" / "Aporte con el Dr. X por [diagnóstico]".
- ✅ "Tenés una actualización en tu ficha. Ingresá para verla." + deep-link autenticado.
- Nada de datos sensibles, tokens ni identificadores sensibles en asunto, cuerpo, URL o `data` del push.
...
- El registro es también auditoría de "se le avisó al participante" — puede tener valor legal.
...
- datos sensibles o diagnóstico en asunto/cuerpo/URL/push.
