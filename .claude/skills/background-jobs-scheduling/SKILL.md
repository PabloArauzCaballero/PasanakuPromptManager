...
4. Zonas con horario de verano: una hora local puede no existir o repetirse. Calculá con una
   librería con base tz, nunca sumando offsets a mano (`contribution-calendar`).
...
1. Al crear/modificar el aporte, `UPSERT` del recordatorio: `(contribution_id, kind, due_at, sent_at NULL)`.
2. Un barrido frecuente envía lo vencido y no enviado, con reclamo atómico (§3).
3. **Revalidá al disparar**: el aporte pudo cancelarse, moverse, o el usuario desactivar el canal.
...
5. Ventana de cortesía: un recordatorio vencido hace demasiado **no se envía**, se marca
   `skipped` (nadie quiere a las 3 AM el aviso de el aporte de ayer). Respetá horas de silencio.
