...
@Component({ selector: 'app-member-list-page' })          // smart
export class MemberListPage {
  private readonly api = inject(MemberApi);
  protected readonly members = this.api.list;             // resource/httpResource
}
// template: <app-member-table [rows]="members.value()" (sort)="..."/>  // dumb
```
...
readonly kind = input<'organizador' | 'group' | 'bank' | 'organizer' | ...>();
// template: @switch (kind()) { @case ('organizador') {...} @case ('group') {...} ... }
```
```ts
...
export class ContributionsPage {
  ...
  load() { return this.http.get('/api/contributions'); } // URL y transporte adentro
}
```
```ts
// ✅ depende de un puerto; la implementación se provee por DI
export abstract class ContributionsGateway { abstract list(): Observable<Contribution[]>; }

@Component({ /* ... */ })
export class ContributionsPage {
  private readonly gateway = inject(ContributionsGateway);  // abstracción
}
```
