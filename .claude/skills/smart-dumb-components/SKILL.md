...
@Component({
  ...
  template: `
    <article>
      ...
      <button (click)="confirm.emit(contribution().id)">Confirmar</button>
    </article>`,
})
export class ContributionCard {
  readonly contribution = input.required<ContributionVm>();
...
@Component({
  ...
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    @for (a of contributions.value(); track a.id) {
      <app-contribution-card [contribution]="a" (confirm)="onConfirm($event)"/>
    }`,
  imports: [ContributionCard],
})
export class CalendarioPage {
  private readonly gateway = inject(ContributionsGateway);
  protected readonly contributions = this.gateway.today;   // resource/httpResource
...
- **Dumb que conoce el dominio de negocio**: recibe un `ContributionVm`, no sabe de reglas de
  calendario ni de permisos.
