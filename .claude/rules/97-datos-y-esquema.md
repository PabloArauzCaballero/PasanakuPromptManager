...
2. **Prohibido inventar relaciones** entre catálogos (por ejemplo, qué códigos de rechazo de la
   pasarela corresponden a qué motivo nuestro, o qué feriado aplica a qué plaza) sin un dataset o
   una definición de negocio que las respalde. Si no hay fuente, se registra la ambigüedad y no se
   adivina.
...
4. **Prohibido inferir parámetros financieros** — tasas, comisiones, topes de cobertura, plazos
   legales, feriados bancarios. Requieren fuente con procedencia y confirmación del responsable de
   negocio o de cumplimiento. Ver `money-movement-safety` y `regulatory-compliance-mapping`.
...
`money-movement-safety` · `synthetic-test-data-generation` · `python-tooling-standards`
